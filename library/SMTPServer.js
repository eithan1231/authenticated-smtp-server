...
const temporaryEmailFile = (id) => path.join(os.tmpdir(), `email-${id}.eml`)

class SMTPServerImplemtnation extends SMTPServer {
  constructor (options) {
    options.banner = 'github:eithan1231/authenticated-smtp-server'
  ...
  */
  onData (stream, session, cb) {
    ...
    // 'end' event
    stream.on('end', () => {
      // adding mail to delivery queue
      agenda.now('jobDistributeMail', {
        id: session.id,
...
module.exports = SMTPServerImplemtnation
